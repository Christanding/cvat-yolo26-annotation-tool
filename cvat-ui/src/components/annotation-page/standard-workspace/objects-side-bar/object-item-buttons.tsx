...
function NavigatePrevKeyframe(props: Props): JSX.Element {
    ...
    return navigatePrevKeyframe ? (
        <CVATTooltip title={`上一个关键帧 ${prevKeyFrameShortcut}`}>
...
function NavigateNextKeyframe(props: Props): JSX.Element {
    ...
    return navigateNextKeyframe ? (
        <CVATTooltip title={`下一个关键帧 ${nextKeyFrameShortcut}`}>
...
function SwitchLock(props: Props): JSX.Element {
    ...
    return (
        <CVATTooltip title={`${locked ? '解锁' : '锁定'}标注框 ${switchLockShortcut}`}>
            {locked ? (
...
function SwitchOccluded(props: Props): JSX.Element {
    ...
    return (
        <CVATTooltip title={`切换遮挡状态 ${switchOccludedShortcut}`}>
            {occluded ? (
                <TeamOutlined
...
function SwitchPinned(props: Props): JSX.Element {
    ...
    const style = locked ? disabledStyle : {};
    return (
        <CVATTooltip title={`${pinned ? '取消固定' : '固定'}标注框 ${switchPinnedShortcut}`}>
            {pinned ? (
                <PushpinFilled
...
function SwitchHidden(props: Props): JSX.Element {
    ...
    return (
        <CVATTooltip title={`${hidden ? '显示' : '隐藏'}标注框 ${switchHiddenShortcut}`}>
            {hidden ? (
                <EyeInvisibleFilled
...
function SwitchOutside(props: Props): JSX.Element {
    ...
    return (
        <CVATTooltip title={`切换画面外状态 ${switchOutsideShortcut}`}>
            {outside ? (
                <Icon
...
function SwitchKeyframe(props: Props): JSX.Element {
    ...
    return (
        <CVATTooltip title={`切换关键帧状态 ${switchKeyFrameShortcut}`}>
            {keyframe ? (
                <StarFilled
                    style={style}
